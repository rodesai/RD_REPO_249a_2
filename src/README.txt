...
The time delay when operating in real time is accomplished using an activity that is executed hourly. That is, each time RealToVirtualTimeActivity is executed, it sleeps for a certain time duration. After this, it schedules itself to run again the next hour. This effectively scales the execution time.


The real time and virtual time can be advanced by the following commands, respectively.
